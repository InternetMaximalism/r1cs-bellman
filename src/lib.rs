...
impl<F: Field, E: Engine, C:FieldConverter<F,E>> Circuit<E::Fr> for WrappedCircuit<F, E, C> {
    fn synthesize<CS: ConstraintSystem<E::Fr>>(self, cs: &mut CS) -> Result<(), SynthesisError> {
        ...
        for constraint in gadget.constraints {
            ...
            let a_lc = Self::_convert_lc::<CS>(cs, a, &witness_map, &public_inputs);
            let b_lc = Self::_convert_lc::<CS>(cs, b, &witness_map, &public_inputs);
            let c_lc = Self::_convert_lc::<CS>(cs, c, &witness_map, &public_inputs);
            cs.enforce(
...
impl<F: Field, E: Engine, C:FieldConverter<F,E>> WrappedCircuit<F, E, C> {
    ...
    fn _convert_lc<CS: ConstraintSystem<E::Fr>>(
        ...
        for (wire, coeff) in exp.coefficients() {
            ...
            let var = Self::_convert_wire::<CS>(cs, *wire, witness_map, public_inputs);
            sum = sum + (fr, var);
        }
        sum
    }
    
    fn _convert_wire<CS: ConstraintSystem<E::Fr>>(
        cs: &mut CS,
        wire: Wire,
        witness_map: &BTreeMap<u32,E::Fr>,
        public_inputs: &HashSet<Wire>
...
mod tests {
    ...
    fn valid_proof() {
        ...
        //1*6 = 6
        witness_map.insert(1,Bls12_381Converter::convert_field(&Element::from(1u8)));
