use franklin_crypto::bellman::pairing::{
    bn256::{Bn256, FrRepr},
    ff::{PrimeField, ScalarEngine},
    Engine,
};
use franklin_crypto::bellman::{
    Circuit, ConstraintSystem, LinearCombination, SynthesisError, Variable,
};
use num::{BigUint, Integer, One, ToPrimitive};
use r1cs::{Constraint, Element, Expression, Field, Gadget, Wire, WitnessGenerator};
use std::collections::{BTreeMap, HashSet};
use std::iter::FromIterator;
use std::marker::PhantomData;
use std::str::FromStr;

/// The BN128 curve.
#[derive(Clone, Debug)]
pub struct Bn128F {}

impl r1cs::Field for Bn128F {
    fn order() -> BigUint {
        BigUint::from_str(
            "21888242871839275222246405745257275088548364400416034343698204186575808495617",
        )
        .unwrap()
    }
}

pub trait FieldConverter<F: Field, E: Engine> {
    fn convert_field(n: &Element<F>) -> E::Fr;
}

pub struct WrappedCircuit<F: Field, E: Engine, C: FieldConverter<F, E>> {
    gadget_constraints: Vec<Constraint<F>>,
    witness_map: BTreeMap<u32, E::Fr>,
    public_inputs: Vec<Wire>,
    _c: PhantomData<C>,
}

impl<F: Field + Clone, E: Engine, C: FieldConverter<F, E>> Clone for WrappedCircuit<F, E, C> {
    fn clone(&self) -> Self {
        Self {
            gadget_constraints: self.gadget_constraints.clone(),
            witness_map: self.witness_map.clone(),
            public_inputs: self.public_inputs.clone(),
            _c: std::marker::PhantomData,
        }
    }
}

impl<F: Field, E: Engine, C: FieldConverter<F, E>> Circuit<E> for WrappedCircuit<F, E, C> {
    fn synthesize<CS: ConstraintSystem<E>>(self, cs: &mut CS) -> Result<(), SynthesisError> {
        let WrappedCircuit {
            gadget_constraints,
            witness_map,
            public_inputs,
            _c,
        } = self;
        let public_inputs = HashSet::from_iter(public_inputs);
        let mut variable_map: BTreeMap<Wire, Variable> = BTreeMap::<Wire, Variable>::new();
        /*for i in 0..n_wire {
            let wire_index = (i+1) as u32;
            let wire = Wire{index:wire_index};
            let variable = Self::_generate_variable(cs, wire, &witness_map, &public_inputs);
            variable_map.insert(wire, variable);
        }*/
        let mut i = 0;
        for constraint in gadget_constraints {
            let Constraint { a, b, c } = constraint;
            let a_lc =
                Self::_convert_lc::<CS>(cs, a, &mut variable_map, &witness_map, &public_inputs);
            let b_lc =
                Self::_convert_lc::<CS>(cs, b, &mut variable_map, &witness_map, &public_inputs);
            let c_lc =
                Self::_convert_lc::<CS>(cs, c, &mut variable_map, &witness_map, &public_inputs);
            cs.enforce(
                || format!("generated by r1cs-bellman at {}", i),
                |_| a_lc,
                |_| b_lc,
                |_| c_lc,
            );
            i += 1;
        }
        Ok(())
    }
}

impl<F: Field, E: Engine, C: FieldConverter<F, E>> WrappedCircuit<F, E, C> {
    pub fn new(
        gadget: Gadget<F>,
        witness_map: BTreeMap<u32, E::Fr>,
        public_inputs: Vec<Wire>,
    ) -> Self {
        Self {
            gadget_constraints: gadget.constraints,
            witness_map,
            public_inputs,
            _c: PhantomData,
        }
    }

    pub fn gadget_constraints(&self) -> &Vec<Constraint<F>> {
        &self.gadget_constraints
    }

    pub fn witness_map(&self) -> &BTreeMap<u32, E::Fr> {
        &self.witness_map
    }

    pub fn public_inputs(&self) -> &Vec<Wire> {
        &self.public_inputs
    }

    fn _convert_lc<CS: ConstraintSystem<E>>(
        cs: &mut CS,
        exp: Expression<F>,
        variable_map: &mut BTreeMap<Wire, Variable>,
        witness_map: &BTreeMap<u32, E::Fr>,
        public_inputs: &HashSet<Wire>,
    ) -> LinearCombination<E> {
        // This is inefficient, but bellman doesn't expose a LinearCombination constructor taking an
        // entire variable/coefficient map, so we have to build one up with repeated addition.
        let mut sum = LinearCombination::zero();
        for (wire, coeff) in exp.coefficients() {
            let fr = C::convert_field(coeff);
            let var = match variable_map.get(wire) {
                Some(variable) => *variable,
                None => {
                    let new_val = Self::_generate_variable(cs, *wire, witness_map, public_inputs);
                    variable_map.insert(*wire, new_val);
                    new_val
                }
            };
            sum = sum + (fr, var);
        }
        sum
    }

    fn _generate_variable<CS: ConstraintSystem<E>>(
        cs: &mut CS,
        wire: Wire,
        witness_map: &BTreeMap<u32, E::Fr>,
        public_inputs: &HashSet<Wire>,
    ) -> Variable {
        let wire_index = wire.index;
        let witness = witness_map.get(&wire_index);
        let is_public = public_inputs.contains(&wire);

        match witness {
            Some(wtns) => {
                if is_public {
                    cs.alloc_input(|| "public input", || Ok(*wtns)).unwrap()
                } else {
                    cs.alloc(|| "private input", || Ok(*wtns)).unwrap()
                }
            }
            None => {
                if is_public {
                    cs.alloc_input(
                        || "public input",
                        || Ok(<E as ScalarEngine>::Fr::from_str("0").unwrap()),
                    )
                    .unwrap()
                } else {
                    cs.alloc(
                        || "private input",
                        || Ok(<E as ScalarEngine>::Fr::from_str("0").unwrap()),
                    )
                    .unwrap()
                }
            }
        }
    }
}

pub struct Bn256Converter;

impl FieldConverter<Bn128F, Bn256> for Bn256Converter {
    fn convert_field(n: &Element<Bn128F>) -> <Bn256 as ScalarEngine>::Fr {
        let n = n.to_biguint();
        // Bls12::Fr::FrRepr's chunks are little endian.
        let u64_size = BigUint::one() << 64;
        let chunks = [
            n.mod_floor(&u64_size).to_u64().unwrap(),
            (n >> 64).mod_floor(&u64_size).to_u64().unwrap(),
            (n >> 64 * 2).mod_floor(&u64_size).to_u64().unwrap(),
            (n >> 64 * 3).mod_floor(&u64_size).to_u64().unwrap(),
        ];
        println!("chunks: {:?}", chunks);
        <<Bn256 as ScalarEngine>::Fr as PrimeField>::from_repr(FrRepr(chunks)).unwrap()
    }
}

#[cfg(test)]
mod tests {
    use franklin_crypto::bellman::groth16::{
        create_random_proof, generate_random_parameters, prepare_verifying_key, verify_proof, Proof,
    };
    use franklin_crypto::bellman::pairing::bn256::{Bn256, Fr};
    use r1cs::{Element, Expression, GadgetBuilder};
    use rand::thread_rng;
    use std::collections::BTreeMap;
    use std::marker::PhantomData;

    use crate::{Bn128F, Bn256Converter, FieldConverter, WrappedCircuit};

    #[test]
    fn valid_proof() {
        let rng = &mut thread_rng();

        // Generate random parameters.
        let empty_map = BTreeMap::<u32, Fr>::new();
        let circuit = build_circuit(empty_map);
        let params = generate_random_parameters::<Bn256, _, _>(circuit, rng).unwrap();
        let pvk = prepare_verifying_key(&params.vk);

        // Generate a random proof.
        let mut witness_map = BTreeMap::<u32, Fr>::new();
        //1*6 = 6
        witness_map.insert(1, Bn256Converter::convert_field(&Element::from(2u8)));
        witness_map.insert(2, Bn256Converter::convert_field(&Element::from(3u8)));
        witness_map.insert(3, Bn256Converter::convert_field(&Element::from(6u8)));
        let circuit = build_circuit(witness_map);
        let proof = create_random_proof(circuit, &params, rng).unwrap();

        // Serialize and deserialize the proof.
        let mut proof_out = vec![];
        proof.write(&mut proof_out).unwrap();
        let proof = Proof::read(&proof_out[..]).unwrap();

        // Verify the proof.
        let public_inputs = &[Bn256Converter::convert_field(&Element::from(6u8))];
        verify_proof(&pvk, &proof, public_inputs).unwrap();
        assert!(verify_proof(&pvk, &proof, public_inputs).is_ok());
    }

    /*#[test]
    fn invalid_proof() {
        let rng = &mut thread_rng();

        // Generate random parameters.
        let empty_map = BTreeMap::<u32,Fr>::new();
        let circuit = build_circuit(empty_map);
        let params = generate_random_parameters::<Bn256, _, _>(circuit, rng).unwrap();
        let pvk = prepare_verifying_key(&params.vk);

        // Generate a random proof.
        let mut witness_map = BTreeMap::<u32,Fr>::new();
        // 2*6 != 6
        witness_map.insert(1,Bn256Converter::convert_field(&Element::from(3u8)));
        witness_map.insert(2,Bn256Converter::convert_field(&Element::from(6u8)));
        witness_map.insert(3,Bn256Converter::convert_field(&Element::from(32u8)));
        let circuit = build_circuit(witness_map);
        let proof = create_random_proof(circuit, &params, rng).unwrap();

        // Serialize and deserialize the proof.
        let mut proof_out = vec![];
        proof.write(&mut proof_out).unwrap();
        let proof = Proof::read(&proof_out[..]).unwrap();

        // Verify the proof.
        let public_inputs = &[Bn256Converter::convert_field(&Element::from(32u8))];
        assert!(verify_proof(&pvk, &proof, public_inputs).is_ok());
    }*/

    fn build_circuit(
        witness_map: BTreeMap<u32, Fr>,
    ) -> WrappedCircuit<Bn128F, Bn256, Bn256Converter> {
        let mut builder = GadgetBuilder::<Bn128F>::new();
        let x = builder.wire();
        let y = builder.wire();
        let z = builder.wire();
        builder.assert_product(
            &Expression::from(&x),
            &Expression::from(&y),
            &Expression::from(&z),
        );
        let gadget = builder.build();
        WrappedCircuit {
            gadget_constraints: gadget.constraints,
            witness_map,
            public_inputs: vec![z],
            _c: PhantomData,
        }
    }

    /*fn convert_bls12_381(n: &Element<r1cs::Bls12_381>) -> <Bls12 as Engine>::Fr {
        let n = n.to_biguint();
        // Bls12::Fr::FrRepr's chunks are little endian.
        let u64_size = BigUint::one() << 64;
        let chunks = [
            n.mod_floor(&u64_size).to_u64().unwrap(),
            (n >> 64).mod_floor(&u64_size).to_u64().unwrap(),
            (n >> 64 * 2).mod_floor(&u64_size).to_u64().unwrap(),
            (n >> 64 * 3).mod_floor(&u64_size).to_u64().unwrap(),
        ];
        <Bls12 as Engine>::Fr::from_repr(bls12_381::Scalar::from_raw(chunks).to_bytes()).unwrap()
    }*/
}
